...
export const createCoffee = async (req, res) => {
    ...
    const newCoffee = new Coffee({ title, description, boughtLocation, flavor, rating });
...
export const getCoffees = async (req, res) => {
    try {
        return res.status(200).json({ coffees: await Coffee.find({})});
    } catch(error) {
        return res.status(error.status).json({ error: true, message: 'Error getting Coffees' });
    }
}

export const getCoffeeByGroup = async (req, res) => {
    try {
